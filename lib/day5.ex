defmodule Day5 do
  ...
  def solve2(example \\ false) do
    ...
    # Run it!
    Enum.map(instructions, &move(&1, stacks, true))
  ...
  def move([n, x, y], stacks, is9001 \\ false) do
    crates =
      cond do
        is9001 -> Agent.get(Enum.at(stacks, x), fn x -> x end) |> Enum.take(n)
        !is9001 -> Agent.get(Enum.at(stacks, x), fn x -> x end) |> Enum.take(n) |> Enum.reverse()
      end
